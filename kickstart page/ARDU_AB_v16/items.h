...
void drawList()
{
  ...
  for (byte i = 0; i < 8; i++)
  {
    if (bitRead (player.hasStuff[(2 * (gameState - STATE_GAME_ITEMS))], i))
    {
      ...
      {
        fillWithWord(0, 81);
        drawTextBox(68, 9 + (6 * positionText), BLACK, TEXT_NOROLL);
      }
      ...
      if (positionText == cursorY)
      {
        switch (gameState)
        {
          case STATE_GAME_ITEMS:
            fillWithSentence(0, 19 + i);
            break;
          case STATE_GAME_WEAPON:
            fillWithSentence(0, 27);
            fillWithNumber(15,i+1);
            break;
          case STATE_GAME_ARMOR:
            fillWithSentence(0, 28);
            fillWithNumber(15,i+1);
            break;
          case STATE_GAME_AMULET:
            fillWithSentence(0, 29 + i);
            break;

        }
        drawTextBox(6, 59, WHITE, TEXT_NOROLL);
      }
      positionText++;
    }
  }
  ...
  {
    fillWithSentence(0, gameState + 3);
