...
void checkRegion()
{
  player.currentRegion = getRegion(player.x / 96, player.y / 96);
...
byte getTileID(int world_pos_x, int world_pos_y)
{
  ...
  byte tile = pgm_read_byte(chunks + ((chunk & 0x7F) * 36) + (tile_pos_x) + (tile_pos_y) * 6);
  if (chunk < 127)
  {
...
void drawTiles()
{
  for (int x = 0; x <= 128; x += 16)
  {
    ...
    {
      byte tile = getTileID(cam.x + x, cam.y + y);
      sprites.drawOverwrite((((cam.x >> 4) << 4) + x) - cam.x, (((cam.y >> 4) << 4) + y) - cam.y, tileSheet, tile);
    }
  }
}
